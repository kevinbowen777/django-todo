...
If you would like to create "test" or "dummy" users, see the :doc:`create_new_users` and :doc:`create_new_tasks` sections for examples.
...
The ``lint``, ``safety``, and ``tests`` are enabled to be run with ``nox -s tests``. Running ``coverage`` and generating documentation, (e.g. ``nox -s docs-3.13``) need to be run explicitly.
